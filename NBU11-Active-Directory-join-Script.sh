...
USER_BASE_DN="DC=cohesitylabs,DC=az"
GROUP_BASE_DN="DC=cohesitylabs,DC=az"
BIND_DN="CN=Administrator,CN=Users,DC=cohesitylabs,DC=az"
...
if $DRY_RUN; then
    echo "[DRY-RUN] Validation not executed."
else
    read -p "Press Enter to validate or Ctrl+C to cancel..."
    "${VALIDATE_CMD[@]}"
fi
